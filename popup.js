...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // アクティブなタブで授業数をカウントする関数
  async function countCoursesOnPage() {
    try {
      // 現在のアクティブなタブを取得
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      ...
      if (response.success) {
        // 結果を表示
        resultDiv.textContent = `ユニークな授業数: ${response.count}`;
        ...
        response.courses.forEach(course => {
          ...
          clickableContainer.className = 'clickable-container';

          // 授業名を表示
          const courseName = document.createElement('div');
          courseName.className = 'course-name';
          courseName.textContent = course.name;
          clickableContainer.appendChild(courseName);

          // Excelデータから教室情報を取得
          const courseData = excelData ? excelData.find(data => data['科目番号'] === course.name) : null;
          const roomFromExcel = courseData ? courseData['教室'] : null;
          ...
          // 教室情報を表示
          if (roomFromExcel) {
            roomInfo.textContent = `教室: ${roomFromExcel}`;
            roomInfo.style.color = '#4CAF50'; // Excelからのデータを緑色で表示
          } else if (course.rooms && course.rooms.length > 0) {
            roomInfo.textContent = `教室: ${course.rooms.join(', ')}`;
          } else {
            roomInfo.textContent = '教室情報なし';
            roomInfo.style.color = '#999';
          }

          // 科目番号を表示する要素を作成
          const codeInfo = document.createElement('div');
          codeInfo.className = 'code-info';
          codeInfo.style.marginTop = '4px';
          codeInfo.style.fontSize = '12px';
          codeInfo.style.color = '#666';
          clickableContainer.appendChild(codeInfo);

          // クリックイベントを追加
          clickableContainer.addEventListener('click', async () => {
            const response = await clickCourse(course.name);
            if (response && response.code) {
              codeInfo.textContent = `科目番号: ${response.code}`;
              codeInfo.style.color = '#4CAF50';
            }
          });

          clickableContainer.appendChild(roomInfo);



          courseItem.appendChild(clickableContainer);
